...
app.post("/api/v1/content", auth, async (req: Request & { userId?: string }, res: Response) => {
...
app.get("/api/v1/content", auth, async (req: Request & { userId?: string }, res: Response) => {
  const userId = req.userId;
  const content = await ContentModel.find({ userid: userId }).populate("userid", "username");
  res.json({ content });
});

app.delete("/api/v1/content", auth, async (req: Request & { userId?: string }, res: Response) => {
  const { contentId } = req.body;
  await ContentModel.deleteMany({ _id: contentId, userid: req.userId });
  res.json({ message: "Content deleted" });
});

app.post("/api/v1/share",auth, async (req: Request &{userId?:string}, res: Response) => {
